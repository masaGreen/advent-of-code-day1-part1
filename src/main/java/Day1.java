...
public class Day1 {
    public static void main(String[] args) {
        InnerDay1 innerDay1 = new InnerDay1();

        List<String> stringInput = innerDay1.getList();
        ...
        for(String s:stringInput){
            ...
            for(char i: s.toCharArray()){
                //for every word pick out the digit chars and add them to a list , each elementr will rep
                // numbers for every word
                if(Character.isDigit(i)){
                    innerList.add(String.valueOf(i));
                }
            }
            //if the word had two digits
            if(innerList.size() == 2){
                ints.add(Integer.parseInt(innerList.get(0) + innerList.get(1)));
            }
            //if the word had only one digit
            if(innerList.size() == 1){
                ints.add(Integer.parseInt(innerList.get(0)+innerList.get(0)));
            }
            //if the word had more than two digits you take the first and last
            if(innerList.size()>2){
                ints.add(Integer.parseInt(innerList.get(0)+innerList.get(innerList.size()-1)));
            }
        }
